...
Proof.
  ...
  rewrite <- (mul_fst_egcd r n) by auto.
  rewrite !Z.mul_mod_idemp_r by auto.
  rewrite !Z.mul_assoc.
  now rewrite <- Z.mul_mod_idemp_l, eq, Z.mul_mod_idemp_l by auto.
Qed.
  ...
  assert (Permutation (map (fun ai => ai * a mod n) rprimes) rprimes).
  {
    ...
    - intros x.
      ...
      + apply in_map_iff.
        ...
        rewrite Z2Nat.id in * by lia.
        exists (x * fst (egcd a n) mod n).
        split.
        * rewrite Z.mul_mod_idemp_l by lia.
          rewrite <- Z.mul_assoc.
          rewrite (Z.mul_comm _ a).
          rewrite <- Z.mul_mod_idemp_r by lia.
          rewrite mul_fst_egcd by auto.
          rewrite Z.mul_mod_idemp_r, Z.mul_1_r by lia.
          apply Z.mod_small; lia.
        * apply In_rel_primes.
          rewrite Z2Nat.id by lia.
          split; [|pose proof (Z.mod_pos_bound (x * (fst (egcd a n))) n); lia].
          apply rel_prime_mod; [lia|].
          apply rel_prime_sym.
          apply rel_prime_mult; [now apply rel_prime_sym|].
          apply rel_prime_sym.
          now apply rel_prime_egcd.
