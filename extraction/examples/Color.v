...
MetaCoq Run (p <- tmQuoteRecTransp main false ;;
             tmDefinition "main_env" p.1).
...
Definition escrow_extract :=
  extract_template_env
    no_check
    main_env
    (KernameSet.singleton <%% main %%>)
    (fun kn => false).
...
Instance EscrowMidlangBoxes : MidlangPrintConfig :=
  {| term_box_symbol := "()";
     type_box_symbol := "()";
     any_type_symbol := "()";
     print_full_names := false; (* full names to avoid clashes*)|}.

Time Compute
    (env <- escrow_extract ;;
     '(_, s) <- finish_print (print_env env (fun _ => None));;
     ret s).
