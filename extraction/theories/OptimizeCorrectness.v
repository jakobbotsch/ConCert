...
Definition valid_case_masks (ind : inductive) (npars : nat) (brs : list (nat * term)) : bool :=
  ...
  | Some mm =>
    (#|param_mask mm| =? npars) &&
    alli (fun c '(ar, br) =>
            (#|get_branch_mask (ctor_masks mm) (inductive_ind ind) c| <=? ar) &&
            (valid_dearg_mask (get_branch_mask (ctor_masks mm) (inductive_ind ind) c) br)) 0 brs
  | None => true
  end.
...
Definition valid_proj (ind : inductive) (npars arg : nat) : bool :=
  ...
  | Some mm => (#|param_mask mm| =? npars) &&
               ...
               negb (nth arg (get_branch_mask (ctor_masks mm) (inductive_ind ind) 0) false)
  | _ => true
  end.
  ...
  - facts.
      ...
      * destruct (get_mib_masks _) eqn:mm; cycle 1.
        ...
        replace
          (skipn (count_zeros (param_mask m))
                 (masked (param_mask m ++ get_branch_mask (ctor_masks m) (inductive_ind ind) c) (map dearg args)))
          with
            (masked (get_branch_mask (ctor_masks m) (inductive_ind ind) c)
                             (skipn #|param_mask m| (map dearg args))); cycle 1.
        { clear.
          generalize (get_branch_mask (ctor_masks m) (inductive_ind ind) c) as m2.
    ...
    + clear IHev1 IHev2.
      ...
      set (branch_mask := match get_mib_masks (inductive_mind ind) with
                          | Some mm => get_branch_mask (ctor_masks mm) (inductive_ind ind) 0
                          | None => []
                          end).
      ...
      * replace (repeat tBox _) with (masked branch_mask (repeat tBox n)); cycle 1.
        { unfold valid_case_masks in valid_brs_masks.
          ...
          - clear -valid_brs_masks.
            ...
            change (get_branch_mask (ctor_masks m) (inductive_ind ind) 0) with branch_mask in bound.
